...
def validation(df_raw):
    try:
        df = df_raw[['start_date', 'zip_code', 'appts_per_listing']]
        return True
    except KeyError:
        st.write("The database uploaded doesn´t have the right columns.")
        return False
...
def cleaning_data(df):
    ...
    df_out = df_out.groupby(['zip_code', 'start_date']).agg(
        {'appts_per_listing': 'sum'}).reset_index(level=['start_date', 'zip_code'])

    return df_out

def page_upload_database():
    st.title("Upload new database")
    st.write("The new database **must** have this 3 columns: **appts_per_listing** , **start_date** and **zip_code**")
